...
// using code from https://github.com/sa413x/UEFI-Bootloader/blob/master/UefiBootLoader/bootloader.c
EFI_STATUS
LocateFile(
IN	CHAR16*				ImagePath,
...
///
EFI_DRIVER_BINDING_PROTOCOL gUnSBDriverBinding = {
  UnSBDriverBindingSupported,
  UnSBDriverBindingStart,
  UnSBDriverBindingStop,
  UN_SB_VERSION,
  NULL,
  NULL
};
  ...
  )
{
  ...
  Status = gBS->UninstallMultipleProtocolInterfaces (
                  ImageHandle,
                  &gEfiDriverBindingProtocolGuid, &gUnSBDriverBinding,
                  NULL
                  );
  if (EFI_ERROR (Status)) {
    return Status;
